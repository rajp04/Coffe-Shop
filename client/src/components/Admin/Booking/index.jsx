...
function AdminBooking() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('https://coffe-shop-btr1.onrender.com/api/booking');
            ...
            <table className="mt-3 w-full">
              <thead className="text-white bg-[#1C2434]">
                <tr className="bg-transparent">
                  ...
                  <th className="bg-transparent px-2 py-2 border-r dark:bg-gray-900 border-gray-600 text-start">Date</th>
                  <th className="bg-transparent px-2 py-2 border-r dark:bg-gray-900 border-gray-600 text-start">Time</th>
                  <th className="bg-transparent px-2 py-2 border-r dark:bg-gray-900 border-gray-600 text-start">Seats</th>
              ...
              <tbody className='bg-[#1c243488]'>
                {currentItems.map((item, index) => (
                  <tr className='bg-transparent border-t border-gray-600' key={index}>
                    ...
                    <td className='bg-transparent px-2 py-2 border-r text-gray-200 dark:bg-gray-700 border-gray-600 text-start'>{item.date}</td>
                    <td className='bg-transparent px-2 py-2 border-r text-gray-200 dark:bg-gray-700 border-gray-600 text-start'>{item.time}</td>
                    <td className='bg-transparent px-2 py-2 border-r text-gray-200 dark:bg-gray-700 border-gray-600 text-start'>{item.seats}</td>
